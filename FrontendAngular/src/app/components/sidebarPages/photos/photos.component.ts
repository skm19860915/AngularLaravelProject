    ...
    this.fileviewService.downloadFiles({ fileList: urlArray }).subscribe(
        ...
        if (urlArray.length == 1) {
          a.download = urlArray[0]['title'];
            ...
            let tmp = urlArray[0]['url'].split('/');
            if(tmp.length > 1) {
                a.download = tmp[tmp.length -2];
            }
            else {
                let tmpArray = urlArray[0]['url'].split("/");
                if(tmpArray.length >= 2) a.download = tmpArray[tmpArray.length - 2];
                else a.download = 'archive.zip';
            }
        }
        a.click();
