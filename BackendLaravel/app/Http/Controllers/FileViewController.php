...
class FileViewController extends Controller
{
    ...
    public function getFileByCategoryByPath(Request $request)
    {
        $folderPath = $request->input('currentPath');
        $fileList = FileModel::select('unique_id', 'title', 'url', 'thumb_url', 'filename', 'diskspace', 'category', 'is_protected', 'is_picture', 'ext', 'created_at', 'updated_at')
        ->where('folder_path', 'like', $folderPath)
        ->orderby('created_at', 'desc')
        ->get();
        $albumProperty = AlbumModel::select('title', 'is_protected', 'user_id', 'password')
    ...
    //Folder privacy
    public function editFolderPrivacy(Request $request) {        
        $unique_id = $request->input('unique_id');
        $is_protected = $request->input('is_protected');
        $password = $request->input('password');
        $this->currentPath = storage_path('app/').$unique_id;

        AlbumModel::where('path', $unique_id."/")
            ->update([
                'is_protected' => $is_protected,
                'password' => $password
            ]);
        // $this->editAllSubFolderPrivacy($this->currentPath, $unique_id."/", $is_protected, $password);
        
        return true;
    }

    private function editAllSubFolderPrivacy($currentPath, $unique_id, $is_protected, $password) {        
        AlbumModel::where('path', $unique_id)
            ->update([
                'is_protected' => $is_protected,
                'password' => $password
        ...
        foreach ($files as $file) {
            if(!in_array($file, array(".",".."))){
                if(!is_dir($currentPath.'/'.$file)) {
                    FileModel::where('folder_path', $unique_id)
                        ->update([
                            'is_protected' => $is_protected,
                            'password' => $password
                        ]);
                ...
                $this->editAllSubFolderPrivacy($currentPath.'/'.$file, $unique_id.$file.'/', $is_protected, $password);
            }
        }        
    }
    ///Folder privacy end
